...
\begin{enumerate}
    ...
    \tans{It follows that the input impedance $Z_\text{in}$ is approximately equal to $\SI{34}{\ohm}$}
    \item
    ...
    \item
    Since the collector quiescent current approximately changes by \SI{9}{\percent\per\celsius}, it is easily seen that its amount doubles for a temeprature change of \tans{\SI{8}{\celsius}}. This leads to a quiescent collector voltage equal to \SI{20}{\volt} causing the transistor to saturate.
\end{enumerate}

\ex{2.30}  % the last Exercise in chapter 2.
...
\tans{$V_\text{M}=V_\text{E}+\SI{2}{\volt}$ is a quite safe value}
% Here ends Chapter 2.
