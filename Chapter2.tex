...
\ex{2.25}
\begin{circuit}{fig:2.25.1}{Commonon emitter amplifier}
    (0,0) node[npn] (Q) {Q}
    (Q.B) to[short] ++(-1,0) coordinate(RP)
    -- ++(0,-.5) 
    to[R, l=$R_2$] ++(0,-2)
...
meaning that $V_text{B}=\SI{1.1}{\volt}$
...
The value of the input capacitor can be computed by:
...
\[\mans{C_\text{in}=\SI{10.6}{\micro\farad}}\]
\todoex{2.26}
