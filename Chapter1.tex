...
\begin{circuit}{fig:1.3.1}{A basic series circuit.}
    (0,0) to[V=$V$,invert] (0,4)
        ...
        to[R=$R_1$,v=$V_{1}$] (2,2)
        to[R=$R_2$,v=$V_{2}$] (2,0)
>>>>>>> 5f098647d65006ab62944fe057dea9dbac11549a
        to (0,0)
\end{circuit}
By KVL and Ohm's law \[ V = V_{1} + V_{2} = R_{1}\cdot I + R_{2} \cdot I = (R_{1}+R_{2}) \cdot I = R \cdot I \]
where \[\mans{R = R_{1} + R_{2}}\] is the resistance of $R_{1}$ and $R_{2}$ in series. Now, consider a simple parallel resistor circuit.
...
<<<<<<< HEAD
    ...
    \ex{1.22}
    Similar to the example given in Figure 1.73, we can design a voltage divider providing negative clamping voltage. Thus, the design is a combination of two diode clamps showing below.
    \begin{circuit}{fig:1.22.1}{symmetrical clamp design to confine signals to the range -5.6 to +5.6\V.}
        (0,2) node[label={[font=\footnotesize]above:$V_\in$}]{}
        (0,2) to[open,  o-] 
        (0,2) to[R=$R$] (3,2)
            to[R=$R$, *-] (3,0) node[ground]{}
        % (3,0) to [R=$R_\1$, *-] (3,2)
        (3,2) to[short, -o] (5,2)
        to[open, v^>=$V_\out$, o-o] (5,0) node[ground]{}
...
\todoex{1.44} % the last exercise in Chapter 1
>>>>>>> 5f098647d65006ab62944fe057dea9dbac11549a
